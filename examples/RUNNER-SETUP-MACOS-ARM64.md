...
someone has to start Docker Desktop and then the runner.  A job dispatched
while its runner is offline does not fail: it sits as "Queued" for up to 24
hours and is then cancelled by GitHub.  So if a dispatched workflow seems to
hang at "Queued", check that the runner shows "Listening for Jobs"
(foreground) or that the service is running, and check the runner's status
on the repository's Settings → Actions → Runners page ("Idle" means
listening; "Offline" means it is not).
